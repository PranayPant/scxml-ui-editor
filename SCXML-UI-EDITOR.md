...
- _Impact on ELK.js Auto-Layout_: When converting React Flow nodes to ELK.js nested graphs, read dimensions from `node.measured?.width ?? node.width` so ELK accurately accounts for custom state node sizes.
